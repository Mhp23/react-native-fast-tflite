...
then
  ...
  bazel build --config=ios_fat, ios_sim_arm64 -c opt --cxxopt=--std=c++17 //tensorflow/lite/ios:TensorFlowLiteC_framework
  bazel build --config=ios_fat, ios_sim_arm64 -c opt --cxxopt=--std=c++17 //tensorflow/lite/ios:TensorFlowLiteCCoreML_framework
